...
from hilcodec.avocodo import SBD
from hilcodec.functional import PQMF
from hilcodec.modules import NormConv2d
...
class STFTDiscriminator(nn.Module):
    ...
    def __init__(
        ...
        conv_pre_no_weight_norm: bool = False,
...
class FilterBankDiscriminator(nn.Module):
    def __init__(
        ...
        if period == 1:
            ...
            self.pqmf = PQMF(
                subbands=period, taps=taps, beta=beta, cutoff_freq=cutoff_freq
            )
...
class MultiPeriodDiscriminator(torch.nn.Module):
    def __init__(
        self, kernel_size: int = 5, stride: int = 3, norm: str = "weight_norm"
    ):
