...
class STDCT(nn.Module):
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        x = F.conv1d(
            x, self.filter, bias=None, stride=self.hop_size, padding=self.padding
        )
        if self.clip:
            x = x[:, :, :-1]
        return x
...
class MDCT(torch.jit.ScriptModule):
    ...
    def inverse(self, x: Tensor) -> Tensor:
        ...
        return F.conv_transpose1d(
            x, mdct_filter, bias=None, stride=self.N, padding=self.N
        )
...
class STFT(nn.Module):
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        if self.padding > 0:
            x = F.pad(
                x.unsqueeze(0), (self.padding, self.padding), mode=self.pad_mode
            ).squeeze(0)
    ...
    def inverse(self, spec: Tensor) -> Tensor:
        ...
        if not self.center:
            raise NotImplementedError(
                "center=False is currently not implemented. " "Please set center=True"
            )
...
class PQMF(torch.nn.Module):
    def __init__(self, subbands=4, taps=62, cutoff_freq=0.142, beta=9.0):
        super().__init__()
        h_proto = (
            torch.from_numpy(design_prototype_filter(taps, cutoff_freq, beta))
    ...
    def analysis(self, x: Tensor) -> Tensor:
        if x.dim() == 2:  # [B, T] -> [B, 1, T]
            x = x.unsqueeze(1)
        x = F.conv1d(
            x, self.pqmf_filter, None, stride=self.subbands, padding=self.taps // 2
        )
        return x
