...
def get_dct_filter(
    y: Tensor, N: int, win_size: Optional[int], win_type: int, window: Optional[Tensor]
) -> Tensor:
    global dct_filter
...
def istdct(
    ...
    N_win_dtype_device = f"{N}_{win_size}_{y.dtype}_{y.device}"
    window_square = (
        dct_window_square[N_win_dtype_device]
        .view(1, -1, 1)
        .expand(y.size(0), -1, y.size(-1))
    )
...
def mdct(y: Tensor, N: int, normalize: int = False) -> Tensor:
    ...
    if N_dtype_device not in mdct_filter:
        ...
        mdct_filter[N_dtype_device] = torch.cos(
...
def istft(
    ...
    if not center:
        raise NotImplementedError(
            "center=False is not implemented.",
...
def spec_to_mel(
    ...
    nmel_nfft_fmax_norm_dtype_device = (
        str(num_mels) + "_" + str(n_fft) + "_" + str(fmax) + "_" + norm_dtype_device
    )
    if nmel_nfft_fmax_norm_dtype_device not in mel_basis:
        ...
        mel_basis[nmel_nfft_fmax_norm_dtype_device] = torch.from_numpy(mel).to(
            dtype=spec.dtype, device=spec.device
        )
...
def mel_spectrogram(
    ...
    mel = spec_to_mel(
        spec, n_fft, num_mels, sampling_rate, fmin, fmax, clip_val, log, norm
    )
