...
from hilcodec.functional import STFT

from . import CausalSTFT, SConv1d, SConvTranspose1d
...
class SEANetResnetBlock(nn.Module):
    ...
    def __init__(
        ...
        self.pre_scale = (
            (1 + idx * res_scale**2) ** -0.5 if res_scale is not None else None
        )
        ...
        if skip == "identity":
            ...
            self.shortcut = SConv1d(
                dim, dim, kernel_size=1, norm=norm, norm_kwargs=norm_params, bias=bias
            )
        elif skip == "scale":
...
class SpecBlock(nn.Module):
    def __init__(
        ...
        mean: float = 0.0,
        std: float = 1.0,
        res_scale: tp.Optional[float] = 1.0,
        zero_init: bool = True,
        ...
        if spec == "stft":
            if causal:
                self.spec = CausalSTFT(
                    n_fft=n_fft, hop_size=stride, pad_mode=pad_mode, learnable=learnable
                )
            else:
                self.spec = STFT(
                    n_fft=n_fft, hop_size=stride, center=False, magnitude=True
                )
        elif spec == "":
            self.spec = None
            return
...
class SEANetEncoder(nn.Module):
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        wav = x
        x = self.conv_pre(x)
        for block, spec_block, downsample in zip(
