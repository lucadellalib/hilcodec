...
class WeightStandardization:
    def __init__(
        self, name: str, dim: Tuple[int], eps: float, weight: Parameter
    ) -> None:
        self.name = name
        self.eps = eps
    ...
    def compute_weight(self, module: T_module) -> torch.Tensor:
        ...
        weight_standardized = (weight - mean) * torch.rsqrt(
            torch.clamp(var * self.fan_in, min=self.eps)
        )
        if gain is not None:
    ...
    def apply(
        ...
        for _, hook in module._forward_pre_hooks.items():
            if isinstance(hook, WeightStandardization) and hook.name == name:
                raise RuntimeError(
                    "Cannot register two weight_standardize hooks on "
...
def weight_standardization(
    ...
    """
    WeightStandardization.apply(
        module, name, dim, eps, scale, learnable_gain, zero_init
    )
    return module
...
def remove_weight_standardization(module: T_module, name: str = "weight") -> T_module:
    ...
    raise ValueError(
        "weight_standardization of '{}' not found in {}".format(name, module)
    )
