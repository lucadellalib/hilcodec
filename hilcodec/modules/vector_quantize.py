        ...
        if self.training:
            with autocast(enabled=False):
                if self.distributed:
                    ...
                    ema_num_numel, ema_embed_numel = (
                        self.ema_num.numel(),
                        self.ema_embed.numel(),
                    )
                    bucket = torch.empty(
                        ema_num_numel + ema_embed_numel, dtype=dtype, device=device
                    )
                    bucket[:ema_num_numel] = embed_onehot.sum(dim=0)
                    bucket[ema_num_numel:] = (flatten.t().float() @ embed_onehot).view(
                        -1
                    )
                    dist.all_reduce(bucket)
                    ema_num_new = bucket[:ema_num_numel]
                    ema_embed_new = bucket[ema_num_numel:].reshape(
                        self.ema_embed.size(1), self.ema_embed.size(0)
                    )
                else:
                ...
                if self.ema_num_threshold <= 0.0:
                    ema_num = (
                        laplace_smoothing(self.ema_num, self.codebook_size, self.eps)
                        * self.ema_num.sum()
                    )
                else:
...
class ShapeGainCodebook(nn.Module):
    def __init__(
        ...
        self.register_buffer(
            "shape_num", torch.ones(self.shape_codebook_size) * ema_num_initial
        )
        self.register_buffer(
            "gain", torch.rand(gain_codebook_size) * 0.9 + 1.0
        )  # Uniform(0.1, 1.9)
        self.register_buffer(
            "gain_num",
            torch.ones(gain_codebook_size) * ema_num_initial * self.gain_num_ratio,
        )
    ...
    def init_embed_(self, data):
        ...
        shape, cluster_size = kmeans(
            shape_data, self.shape_codebook_size, self.kmeans_iters
        )
        ...
        gain, cluster_size = kmeans(
            gain_data, self.gain_codebook_size, self.kmeans_iters
        )
        gain = gain.squeeze()
        ...
        if self.distributed:
            ...
            gain, shape = bucket[:gain_numel], bucket[gain_numel:].reshape(
                self.shape_codebook_size, -1
            )
    ...
    def expire_codes_(
        self, flatten: torch.tensor, gain_flatten: torch.tensor
    ) -> Tuple[int, int]:
        if self.ema_num_threshold == 0.0:
            return 0
    ...
    def forward(self, x: torch.tensor):
        ...
        x_shape, dtype = (
            x.shape,
            self.shape.dtype,
        )  # x.dtype: float16 / embed.dtype: float32
        flatten = rearrange(
            x, "... d -> (...) d"
        ).float()  # [Batch x Time(BT), Channel(C)]
        ...
        with autocast(enabled=False):
            ...
            shape_onehot = F.one_hot(shape_ind, self.shape_codebook_size).type(
                dtype
            )  # [BT, scs]
            ...
            # gain
            gain_flatten = (flatten * shape_quantize).sum(
                dim=1, keepdim=True
            )  # [BT, 1]
            if self.log_gain:
            ...
            distance = -(  # [BT, 1] x [1, gcs] = [BT, gcs]
                gain_flatten.pow(2) - 2 * gain_flatten @ gain + gain.pow(2)
            )
            gain_ind = distance.max(dim=-1).indices  # [BT]
            gain_onehot = F.one_hot(gain_ind, self.gain_codebook_size).type(
                dtype
            )  # [BT, gcs]
            ...
            if self.training:
                ...
                if self.distributed:
                    # Concatenate multiple tensors before all_reduce for speedup
                    bucket = torch.cat(
                        gain_num_new, shape_num_new, gain_new, shape_new.view(-1)
                    )
                    dist.all_reduce(bucket)
                    ...
                    start = end
                    shape_new = bucket[start:].reshape(
                        self.shape.size(1), self.shape.size(0)
                    )
                shape_new.div_(
                    shape_new.norm(p=2, dim=0, keepdim=True).clamp(min=self.eps)
                )
...
class VectorQuantize(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        self._codebook = codebook_class(**kwargs)
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.layers = nn.ModuleList(
            [
                VectorQuantize(gradient_flow=False, **kwargs)
        ...
        self.dropout_index = (
            dropout_index  # index of VQ layers to choose when dropout==True
        )
