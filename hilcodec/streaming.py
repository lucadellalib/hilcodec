...
from hilcodec.causal_layers import CausalSTFT, SConv1d, SConvTranspose1d
...
class ResBlock(nn.Module):
    ...
    def forward(
        self, x: Tensor, cache: tp.List[Tensor]
...
class SpecBlock(nn.Module):
    def __init__(
        ...
        self.layer = SConv1d(n_fft // 2 + 1, channels, 1, norm=norm, bias=bias)
...
class Encoder(nn.Module):
    def __init__(
        ...
        self.conv_pre = nn.Conv1d(channels, mult * n_filters, kernel_size, bias=bias)
        if norm == "weight_norm":
        ...
        for spec_mean, spec_std, ratio in zip(spec_means, spec_stds, self.ratios):
            ...
            downsample_pointwise = nn.Sequential(
                ...
                SConv1d(
                    mult * n_filters, mult * n_filters * 2, 1, norm=norm, bias=False
                ),
            )
            downsample_depthwise = SConv1d(
                mult * n_filters * 2,
                mult * n_filters * 2,
                kernel_size=ratio * 2,
                stride=ratio,
                groups=mult * n_filters * 2,
                norm=norm,
                bias=bias,
            )
    ...
    def initialize_cache(self, x: Tensor) -> tp.List[Tensor]:
        ...
        wav_cache = torch.zeros(
            x.size(0), 1, self.spec_post.spec.cache_len, device=x.device
        )
    ...
    def forward(self, x: Tensor, *args) -> tp.Tuple[Tensor, tp.List[Tensor]]:
        ...
        for blocks, spec_block, down_pointwise, down_depthwise in zip(
            self.blocks,
            self.spec_blocks,
            self.downsample_pointwise,
            self.downsample_depthwise,
        ):
...
class Decoder(nn.Module):
    def __init__(
        ...
        self.conv_pre_pointwise = SConv1d(
            dimension, mult * n_filters, 1, norm=norm, bias=False
        )
        ...
        for ratio in self.ratios:
            ...
            upsample_depthwise = SConvTranspose1d(
                mult * n_filters,
                mult * n_filters,
                kernel_size=ratio * 2,
                stride=ratio,
            ...
            upsample_pointwise = SConv1d(
                mult * n_filters, mult * n_filters // 2, 1, norm=norm, bias=bias
            )
        ...
        self.conv_post = SConv1d(
            n_filters, channels, last_kernel_size, norm=norm, bias=bias
        )
    ...
    def forward(self, x: Tensor, *args) -> tp.Tuple[Tensor, tp.List[Tensor]]:
        ...
        for blocks, up_act, up_depthwise, up_pointwise in zip(
            self.blocks,
            self.upsample_act,
            self.upsample_depthwise,
            self.upsample_pointwise,
        ):
            x = up_act(x)
...
class HILCodec(nn.Module):
    ...
    def forward(
        self, x: torch.Tensor, n: int, *args
